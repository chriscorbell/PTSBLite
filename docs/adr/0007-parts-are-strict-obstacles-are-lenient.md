...
must agree: an occupant present in one but not the other renders and gets priced yet cannot be erased
or collided with. `CONTEXT.md` names this as the invariant that matters most in the codebase.

Rebuilding a design from a scene — opening a file, clearing parts, applying a build-area change —
...
- Blower and terminal cells took a different path with no occupancy guard at all, and
  `SparseGrid.place` throws on an occupied cell — so a file holding two overlapping terminals did not
  degrade quietly, it crashed the load with an unhandled exception.
...
reachable by using the app — every placement path already refuses both — so a file containing one is
corrupt. Reporting beats repairing: silently dropping part of a saved design is worse than declining
to open it. Rejection is all-or-nothing, so a part that fails partway registers nothing.
...
representable and flagged. Refusing to open the file would leave the user unable to fix the very
problem the validator exists to report.

## Consequences

- `deserializeDesign` returns a typed failure listing every offending part, not just the first: an
  installer opening a corrupt file is better served by the whole list than by discovering them one
  reopen at a time.
